SELECT G.GRN_ID, G.PO_ID, G.IRN_ID, G.STORE_ID FROM INV_GRNS G WHERE G.GRN_NO in (19);

SELECT * FROM INV_STORES WHERE STORE_ID = 80;

update inv_pos set po_status='APPROVED' where po_id=11;

update inv_indents set indent_status='IRN_CREATED'
where indent_id IN (SELECT IG.INDENT_ID FROM INV_GRN_ITEMS IG WHERE IG.GRN_ID = 202);

SELECT IG.RECEIVED_QTY, IG.ITEM_ID, G.COMPANY_ID, G.BRANCH_ID 
FROM INV_GRN_ITEMS IG, INV_GRNS G WHERE G.GRN_ID = IG.GRN_ID AND G.GRN_ID = 202;
...
DECLARE
  cursor rec_item is (  
    ...
    where g.grn_id = gi.grn_id and GI.INDENT_ID = I.INDENT_ID and G.GRN_ID = 202);
BEGIN
...
update inv_irns set irn_status = 'APPROVED' where irn_id in (282);

DELETE FROM INV_GRN_ITEMS WHERE GRN_ID in (202);

DELETE FROM INV_GRNS WHERE GRN_ID in (202);